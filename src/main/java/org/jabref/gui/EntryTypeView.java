...
public class EntryTypeView extends BaseDialog<EntryType> {
	...
	// Added
	@FXML
	private ButtonType add_custom_entry;
	...
	public EntryTypeView(LibraryTab libraryTab, DialogService dialogService, PreferencesService preferences) {
		...
		setResultConverter(button -> {
			// The buttonType will always be "cancel", even if we pressed one of the entry
			// type buttons
			return type;
		});
		...
		btnGenerate.textProperty().bind(EasyBind.map(viewModel.searchingProperty(),
	...
	public void initialize() {
		...
		// we set the managed property so that they will only be rendered when they are
		// visble so that the Nodes only take the space when visible
		...
		if (isBiblatexMode) {
			...
			otherEntries = BibtexEntryTypeDefinitions.add_new_entries();
			otherEntries.addAll(BibtexEntryTypeDefinitions.ALL);
		...
		viewModel.idTextProperty().addListener((obs, oldValue, newValue) -> visualizer
				.initVisualization(viewModel.idFieldValidationStatus(), idTextField, true));
	...
	/**
	 * The description is originating from biblatex manual chapter 2 Biblatex
	 * documentation is favored over the bibtex, since bibtex is a subset of
	 * biblatex and biblatex is better documented.
	 */
	public static String getDescription(EntryType selectedType) {
		if (selectedType instanceof StandardEntryType entryType) {
			...
			case Proceedings -> {
				return Localization
						.lang("A single-volume conference proceedings. This type is very similar to \"Collection\".");
			}
			case TechReport -> {
				return Localization.lang(
